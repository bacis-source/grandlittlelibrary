...
export function ChokoVisionPanel({ noticingId, userId, initialGenerations = [], initialRevisions = [] }: Props) {
  ...
  if (!noticing || !caption) return <section className="choko-vision"><header><span className="choko-avatar"><img src="/choko-von-snack.png" alt=""/></span><div><p className="kicker">Choko Vision</p><h2>Choko noticed…</h2></div></header><p className="choko-intro">A quiet second look at the photograph and your original observation. Nothing is sent until you ask.</p>{error && <p className="error" role="alert">{error}</p>}<button className="primary" disabled={state === 'analyzing'} onClick={generate}>{state === 'analyzing' ? <><RefreshCw className="spin"/> Choko is noticing…</> : <><Eye/> Let Choko notice this</>}</button>{state === 'error' && <p className="ai-state">AI error · Your Noticing was not changed.</p>}</section>

  return <section className="choko-vision"><header><span className="choko-avatar"><img src="/choko-von-snack.png" alt=""/></span><div><p className="kicker">Choko Vision</p><h2>Choko noticed…</h2></div><button className="secondary compact" disabled={state === 'analyzing'} onClick={generate}><RefreshCw className={state === 'analyzing' ? 'spin' : ''}/> {state === 'analyzing' ? 'Noticing…' : 'Regenerate'}</button></header>{error && <p className="error" role="alert">{error}</p>}
