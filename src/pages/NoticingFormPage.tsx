...
import type { AIGeneration, EditorialRevision, FeedbackCategory, NoticingAsset, NoticingInput } from '../types'

const reviewFeedbackOptions: Array<{ value: FeedbackCategory; label: string }> = [
  { value: 'too_descriptive', label: 'Too descriptive' },
  { value: 'too_poetic', label: 'Too poetic' },
  { value: 'missed_main_detail', label: 'Missed the interesting detail' },
  { value: 'wrong_fact', label: 'Wrong fact' },
  { value: 'wrong_tone', label: 'Wrong tone' },
  { value: 'better_wording', label: 'Better wording' },
]
...
export function NoticingFormPage() {
  ...
  function toggleFeedback(category: FeedbackCategory) {
    setFeedbackCategories((current) => current.includes(category) ? current.filter((item) => item !== category) : [...current, category])
  }
    ...
    try {
      ...
      if (id) {
        ...
        if (reviewing) {
          ...
          if (noticingGeneration && payload.observation_text.trim() && noticingRevision?.final_human_text !== payload.observation_text.trim()) decisions.push(saveEditorialRevision({ noticingId: id, userId: session.user.id, generation: noticingGeneration, finalText: payload.observation_text, decision: payload.observation_text.trim() === noticingGeneration.generated_text ? 'accepted' : 'edited', feedbackReason, feedbackCategories }))
          if (captionGeneration && captionText.trim() && captionRevision?.final_human_text !== captionText.trim()) decisions.push(saveEditorialRevision({ noticingId: id, userId: session.user.id, generation: captionGeneration, finalText: captionText, decision: captionText.trim() === captionGeneration.generated_text ? 'accepted' : 'edited', feedbackReason, feedbackCategories }))
