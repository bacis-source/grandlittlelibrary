...
export type FeedbackCategory = 'too_descriptive' | 'too_poetic' | 'too_generic' | 'too_long' | 'too_short' | 'too_explanatory' | 'too_motivational' | 'invented_detail' | 'wrong_fact' | 'missed_main_detail' | 'wrong_tone' | 'too_cute' | 'too_serious' | 'better_wording' | 'other'
...
export interface ChokoAIResult { response_mode?: 'visual_wonder' | 'reflection' | 'nature_fact'; subject_identification?: string; subject_confidence?: number; insight_basis?: string; suggested_title?: string; suggested_tags?: string[]; literal_observation: string; overlooked_detail: string; choko_noticing: string; caption: string; confidence: number; uncertainties: string[] }
